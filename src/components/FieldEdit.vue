...
const saveFieldValues = async(fieldId) => {
    try {
        const valuesToDelete = originalFieldValues.value.filter(original => 
            !fieldValues.value.some(current => current.id === original.id)
        );

        await Promise.all(valuesToDelete.map(value => 
            FieldValueServices.deleteFieldValues(value.id)
        ));

        await Promise.all(fieldValues.value.map((fieldValue) => {
            if(fieldValue.id) {
...
const fetchFieldValues = async() => {
    try{
        let response = await FieldValueServices.getFieldValuesByField(props.field.id)
    ...
    <v-card>
        ...
        <v-form v-model="form" class="ml-4 mr-4">
            ...
            <div v-if="showFieldValues">
                <v-row v-for="(fieldValue, index) in fieldValues" :key="index" class="mr-1 mt-n6">
                    <v-col cols="12" md="11">
                        <v-text-field 
