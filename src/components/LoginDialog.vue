...
const cardTitle = computed(() => { return login.value ? 'Log In' : 'Sign Up' })
const changeFormTitle = computed(() => { return login.value ? 'Create Account' : 'Log In' })
const loginBtnTitle = computed(() => { return login.value ? 'Log In' : 'Sign Up' })
const user = ref({
    firstName: "",
    lastName: "",
    email: "",
...
const rules = {
    ...
    passwordCheck: (value) => user.value.password === value || "Password does not match"
}
